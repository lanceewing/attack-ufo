...
public class Memory {
  ...
  private void initVicMemory(Vic vic, Pia pia) {
    // At $0000 is 1K of 8-bit RAM. The lower half is used for the normal 6502 zero-page 
    // and stack. The second half from $0200-$03FF is used for the Video screen memory.
    mapChipToMemory(new RamChip(), 0x0000, 0x03FF);
    
    // At $0400 is 1K of 4-bit RAM. The lower half is not used, but the top half from
    // $0600 is used for the colour RAM.
    ...
    // At $1400 is the PIA chip used for reading the controls. It only has 4 addressable locations.
   ...
   * used for things such as ROM images.
   * 
   * @param chip The chip to map at the given address range.
   * @param startAddress The start of the address range.
   * @param endAddress The end of the address range.
